...
@bot.command(description="React with an even number of players to make random teams of your chosen size", brief="xd")
async def rteams(ctx, size):
    ...
    if len(users) != 0 and len(users) % size == 0:
        random = []
        ...
        team_number = 1
        team_string = ''
        for j in range(0, len(users), size):
            team_string += "Team {}: {}, {}\n".format(team_number, str(random[j])[:-5], str(random[j + 1])[:-5])
            team_number += 1
